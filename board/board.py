...
def tuple_plus_one(t: Tuple):
	return tuple(i + 1 for i in list(t))


def game_log(Field: Field):
	if Field.winner.color == 1:
		score = "1 - 0"
	elif Field.winner.color == 2:
		score = "0 - 1"
	else:
		score = "1/2 - 1/2"
	Say(f"\n\n\t{str(type(Field.players[0])).split('.')[-1][:-2]}"
		f"\t\t{score}\t\t"
		f"{str(type(Field.players[1])).split('.')[-1][:-2]}\n").info_()

	captures_1 = 0
	captures_0 = 0
	if len(Field.move_history) % 2 == 0:
		moves_num = int(len(Field.move_history) / 2)
	else:
		moves_num = int(len(Field.move_history) / 2) + 1
	...
	for m in range(0, moves_num):
		if (len(Field.move_history) % 2 != 0) and (m == moves_num - 1):
			if Field.capture_history[2 * m]:
				...
				Say(f"{m + 1}.\t"
					  f"{tuple_plus_one(Field.move_history[2 * m])}").info_()
		else:
			if Field.capture_history[2 * m] or \
				...
				Say(f"{m + 1}.\t"
					  f"{tuple_plus_one(Field.move_history[2 * m])}  \t"
					f"{tuple_plus_one(Field.move_history[2 * m + 1])}").info_()
